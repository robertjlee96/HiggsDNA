...
class Systematic:
    ...
    def __init__(self, name, method, branches=None, function=None, sample=None):
        ...
        if not (self.method == "from_branch" or self.method == "from_function"):
            message = (
                "[Systematic : __init__] Systematic: %s, supported methods for constructing a Systematic are 'from_branch' and 'from_function', not '%s' as you passed."
        ...
        if self.method == "from_branch":
            if self.branches is None:
                message = (
                    "[Systematic : __init__] Systematic: %s, method was selected as 'from_branch', but 'branches' argument was not provided."
                    % self.name
                )
                logger.exception(message)
            ...
            if not isinstance(self.branches, dict):
                message = (
                    "[Systematic : __init__] Systematic: %s, for systematics with method 'from_branch', the 'branches' argument must be a dictionary of the form { <variation> : <branch>, ... }, not %s as you have passed."
                    % (self.name, str(type(self.branches)))
                )
                logger.exception(message)
                raise TypeError(message)
        ...
        if self.method == "from_function":
            if self.function is None:
                message = (
                    "[Systematic : __init__] Systematic: %s, method was selected as 'from_function' but 'function' argument was not provide."
                    % self.name
                )
                logger.exception(message)
                raise ValueError(message)
            ...
            if not isinstance(self.function["module_name"], str):
                message = (
                    "[Systematic : __init__] Systematic: %s, when specifying 'from_function', the module containing the function should be given as a string, e.g. 'higgs_dna.systematics.lepton_systematics', not type '%s' as you have given"
                    % (self.name, str(type(self.function["module_name"])))
                )
                logger.exception(message)
                raise TypeError(message)
            ...
            if self.function["module_name"] not in sys.modules:
                message = (
                    "[Systematic : __init__] Systematic: %s, method was selected as 'from_function' with module '%s', but the module has not been imported! Add an import statement to your analysis script."
                    % (self.name, self.function["module_name"])
                )
                logger.exception(message)
            ...
            if not hasattr(self.function["module"], self.function["name"]):
                message = (
                    "[Systematic : __init__] Systematic: %s, method was selected as 'from_function' but function '%s' was not found in module %s."
                    % (self.name, self.function["name"], self.function["module"])
                )
                logger.exception(message)
    ...
    def check_fields(self, events):
        ...
        missing_fields = awkward_utils.missing_fields(
            events, [field for var, field in self.branches.items()]
        )
        if missing_fields:
            message = (
                "[Systematic : check_fields] Systematic: %s, the events array is missing the following fields: %s, which were specified as the input branches to calculate this systematic."
                % (self.name, str(missing_fields))
            )
            logger.exception(message)
            raise ValueError(message)
    ...
    def get_function_kwargs(self, events, central_only=None):
        ...
        function_args = signature(
            getattr(self.function["module"], self.function["name"])
        ).parameters
        ...
        for arg in function_args:
            if arg == "events":
                ...
                if hasattr(self, arg):
                    ...
                    message = "[Systematic : get_function_kwargs] Systematic: %s, for function '%s' from module <%s>, we found an argument %s that is not present as an attribute of this class and we did not know how to otherwise set this argument. This may lead to unintended behavior or crashes!" % (
                        self.name,
                        self.function["name"],
        ...
        logger.debug(
            "[Systematic : get_function_kwargs] Systematic: %s, for function '%s' from module <%s>, we are passing arguments as:"
            % (self.name, self.function["name"], self.function["module"])
        )
...
class WeightSystematic(Systematic):
    ...
    def __init__(
        ...
        self.is_applied = (
            {}
        )  # a weight will often be applied to multiple different sets of events (corresponding to different independent collections)
    ...
    def produce(self, events, central_only=False):
        ...
        if self.method == "from_branch":
            ...
            for variation, branch in self.branches.items():
                awkward_utils.add_field(
                    events, self.name + "_" + variation, events[branch]
                )
            ...
            for variation, values in variations.items():
                if isinstance(
                    variation, str
                ):  # plain field in array, e.g. events.weight_syst1_up
                    name = "weight" + "_" + self.name + "_" + variation
                    self.branches[variation] = name
                elif isinstance(
                    variation, tuple
                ):  # nested field in array, e.g. events.Photon.weight_syst1_up
                    name = tuple(
                        (variation[0], "weight" + "_" + self.name + "_" + variation[1])
                    )
                    self.branches[variation[1]] = name
                logger.debug(
                    "[WeightSystematic : produce] WeightSystematic: %s, adding field %s to events array"
                    % (self.name, name)
                )
        ...
        for variation, branch in self.branches.items():
            ...
            logger.debug(
                "[WeightSystematic : produce] WeightSystematic: %s, variation: %s has mean: %.4f and std. dev. %.4f"
                % (
                    self.name,
                    variation,
                    self.summary[branch]["mean"],
                    self.summary[branch]["std"],
                )
            )
    ...
    def calculate_variations(self, events, central_only):
        ...
        variations = getattr(self.function["module"], self.function["name"])(**kwargs)

        for variation, values in variations.items():
            if not (isinstance(variation, str) or isinstance(variation, tuple)):
                message = (
                    "[WeightSystematic : produce] WeightSystematic: %s, each key in variations dict should be a <str> or <tuple> of <str>, not %s as we got."
                    % (self.name, str(type(variation)))
                )
                logger.exception(message)
                raise TypeError(message)
            ...
            if not isinstance(values, awkward.highlevel.Array):
                message = (
                    "[WeightSystematic : produce] WeightSystematic: %s, each value in variations dict should be an <awkward.highlevel.Array>, not %s as we got."
                    % (self.name, str(type(values)))
                )
                logger.exception(message)
                raise TypeError(message)
    ...
    def apply(
        ...
        if not self.is_produced:
            message = (
                "[WeightSystematic : apply] WeightSystematic: %s must be produced before applying it!"
                % self.name
            )
            logger.exception(message)
            raise ValueError(message)
        ...
        if syst_tag in self.is_applied.keys():
            if self.is_applied[syst_tag]:
                message = (
                    "[WeightSystematic : apply] WeightSystematic: %s has already been applied to events set: %s"
                    % (self.name, syst_tag)
                )
                logger.exception(message)
                raise ValueError(message)
        ...
        for variation, values in weights.items():
            ...
            logger.debug(
                "[WeightSystematic : apply] WeightSystematic: %s, variation: %s, independent collection: %s per-event weight has mean %.4f and std. dev. %.4f"
                % (self.name, variation, syst_tag, mean, std)
            )
        ...
        if self.modify_central_weight:
            if not ("central" in weights.keys()):
                message = (
                    "[WeightSystematic : apply] WeightSystematic: %s, you selected to modify the central event weight, but a central value for this weight systematic was not found (variations are: %s)."
                    % (self.name, str(weights.keys()))
                )
                logger.exception(message)
                raise ValueError(message)
            ...
            logger.debug(
                "[WeightSystematic : apply] WeightSystematic: %s, modifying central weight '%s' by multiplying it by the per-event weight."
                % (self.name, CENTRAL_WEIGHT)
            )
            ...
            if mask is None:
                ...
                logger.debug(
                    "[WeightSystematic : apply] WeightSystematic: %s, a mask was provided, so we will modify central weight only where mask evaluates True."
                    % (self.name)
                )

            weight_masked = awkward.where(mask, weight, awkward.ones_like(weight))

            self.summary[syst_tag]["central"]["frac"] = float(
                awkward.sum(mask)
            ) / float(len(weight))
            if self.summary[syst_tag]["central"]["frac"] < 1.0:
                logger.debug(
                    "[WeightSystematic : apply] WeightSystematic: %s, independent collection: %s, per-event weight is applied to %.2f percent of events."
                    % (
                        self.name,
                        syst_tag,
                        self.summary[syst_tag]["central"]["frac"] * 100.0,
                    )
                )

            self.summary[syst_tag]["central"]["before"] = awkward.mean(
            ...
            self.summary[syst_tag]["central"]["after"] = awkward.mean(
                events[CENTRAL_WEIGHT]
            )

            logger.debug(
                "[WeightSystematic : apply] WeightSystematic: %s, independent collection: %s, mean value of central weight before/after applying: %.4f/%.4f"
                % (
                    self.name,
                    syst_tag,
                    self.summary[syst_tag]["central"]["before"],
                    self.summary[syst_tag]["central"]["after"],
                )
            )
...
class ObjectWeightSystematic(WeightSystematic):
    ...
    def __init__(
        ...
        super(ObjectWeightSystematic, self).__init__(
            name,
            method,
            modify_central_weight,
            branches=branches,
    ...
    def calculate_variations(self, events, central_only):
        ...
        """
        variations = super(ObjectWeightSystematic, self).calculate_variations(
            events, central_only
        )
        if self.method == "from_branch":
            return variations
    ...
    def apply(
        ...
        return super(ObjectWeightSystematic, self).apply(
            events, syst_tag, central_only, mask, add_fields=add_fields
        )
...
class EventWeightSystematic(WeightSystematic):
    ...
    def apply(
        ...
        return super(EventWeightSystematic, self).apply(
            events, syst_tag, central_only, mask, add_fields=add_fields
        )
...
class SystematicWithIndependentCollection(Systematic):
    ...
    def __init__(
        ...
        super(SystematicWithIndependentCollection, self).__init__(
            name, method, branches, function, sample
        )
    ...
    def produce(self, events):
        ...
        if not isinstance(independent_collections, dict):
            message = (
                "[SystematicWithIndependentCollection : produce] SystematicWithIndependentCollection: %s the 'produce' method should return a dictionary of the form { 'variation_name' : modified_events, ... }, not %s as we got."
                % (self.name, str(type(independent_collections)))
            )
            logger.exception(message)
            raise TypeError(message)
        ...
        for variation, events_var in independent_collections.items():
            if not isinstance(variation, str):
                message = (
                    "[SystematicWithIndependentCollection : produce] SystematicWithIndependentCollection: %s, each key in the independent_collections dict should be a <str>, not %s as we got."
                    % (self.name, str(type(variation)))
                )
                logger.exception(message)
                raise TypeError(message)
            ...
            if not isinstance(events_var, awkward.highlevel.Array):
                message = (
                    "[SystematicWithIndependentCollection : produce] SystematicWithIndependentCollection: %s, each value in the independent_collections dict should be an <awkward.highlevel.Array>, not %s as we got."
                    % (self.name, str(type(events_var)))
                )
                logger.exception(message)
                raise TypeError(message)
            ...
            if not (len(events) == len(events_var)):
                message = (
                    "[SystematicWithIndependentCollection : produce] SystematicWithIndependentCollection: %s, each independent collection should have the same number of events as the input events array. We got len(events) = %d, len(independent_collection) = %d"
                    % (self.name, len(events), len(events_var))
                )
                logger.exception(message)
                raise ValueError(message)
            ...
            if not (set(events.fields) == set(events_var.fields)):
                message = (
                    "[SystematicWithIndependentCollection : produce] SystematicWithIndependentCollection: %s, each independent collection should have the same fields as the input events array. We got events.fields = %s, independent_collection.fields = %s"
                    % (self.name, events.fields, events_var.fields)
                )
                logger.exception(message)
                raise ValueError(message)
    ...
    def produce_from_branch(self, events):
        ...
        for variation, branch in self.branches.items():
            ...
            independent_collections[variation] = awkward.with_field(
                events, new_branch_content, self.branch_modified
            )
    ...
    def produce_from_function(self, events):
        ...
        for variation, branch in variations.items():
            ...
            if not variation == "nominal" and self.nominal_only:
                continue
            independent_collections[variation] = awkward.with_field(
                events, branch, self.branch_modified
            )
