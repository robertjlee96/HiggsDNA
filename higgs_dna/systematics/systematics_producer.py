from higgs_dna.systematics.systematic import (
...
class SystematicsProducer:
    ...
    def syst_is_relevant(self, syst, syst_info):
        ...
        if "years" not in syst_info.keys() and "samples" not in syst_info.keys():
            ...
            if (
                self.sample.year in syst_info["years"]
                and self.sample.process in syst_info["samples"]
            ):
                return True

        logger.debug(
            "[SystematicsProducer : syst_is_relevant] SystematicsProducer '%s' found that syst '%s' is not applicable for year '%s' and sample '%s'."
            % (self.name, syst, self.sample.year, self.sample)
        )
    ...
    def add_systematics(self, systematics):
        ...
        if "weights" in systematics.keys():
            for syst, syst_info in systematics["weights"].items():
                ...
                if self.sample is not None:
                    if (
                        self.sample.is_data
                    ):  # weight systs should never be applicable to data
                        continue

                    if not self.syst_is_relevant(
                        syst, syst_info
                    ):  # otherwise some weight systs might only be applicable to certain MC samples
                        continue

                syst_info = self.convert_lists_to_tuples(syst_info)

                logger.info(
                    "[SystematicsProducer : add_systematics] SystematicsProducer '%s', adding weight systematic '%s'."
                    % (self.name, syst)
                )
                logger.debug(
                    "[SystematicsProducer : add_systematics] Details: %s" % syst_info
                )
                ...
                self.weights[
                    ...
                    []
                )  # a single weight syst in the config can result in multiple systematics if it has multiple target collections

                if syst_info["type"] == "object":
                    if "input_collection" in syst_info.keys():
                        syst_info["branches"] = None
                        if "target_collections" not in syst_info.keys():
                            logger.debug(
                                "[SystematicsProducer : add_systematics] No target collections specified for syst '%s', using the input collection '%s' as the target"
                                % (syst, syst_info["input_collection"])
                            )
                            syst_info["target_collections"] = [
                                syst_info["input_collection"]
                            ]
                    ...
                    for target_collection in syst_info["target_collections"]:
                        ...
                        if "modifies_taggers" in syst_info.keys():
                            if isinstance(
                                syst_info["modifies_taggers"], dict
                            ):  # you gave a map of target_collection : taggers
                                weight_syst.modifies_taggers = syst_info[
                                    "modifies_taggers"
                                ][target_collection]
                            elif isinstance(
                                syst_info["modifies_taggers"], list
                            ):  # you gave a list of taggers
                                weight_syst.modifies_taggers = syst_info[
                                    "modifies_taggers"
                                ]
                    ...
                    if "requires_branches" not in syst_info.keys():
        ...
        if "independent_collections" in systematics.keys():
            for syst, syst_info in systematics["independent_collections"].items():
                if (
                    syst in self.independent_collections.keys()
                ...
                if self.sample is not None:
                    if (
                        self.sample.is_data
                    ):  # FIXME: need to allow for ic systs to modify central value in data but not result in ICs with variations
                        continue

                syst_info = self.convert_lists_to_tuples(syst_info)

                logger.info(
                    "[SystematicsProducer : add_systematics] SystematicsProducer '%s', adding systematic with independent collection '%s'."
                    % (self.name, syst)
                )
                logger.debug(
                    "[SystematicsProducer : add_systematics] Details: %s" % syst_info
    ...
    def produce(self, events):
        ...
        for name, weight_systs in self.weights.items():
            logger.debug(
                "[SystematicsProducer : produce] Producing weights for weight variation: %s"
                % name
            )
            ...
            if hasattr(weight_systs[0], "input_collection"):
                missing_fields = awkward_utils.missing_fields(
                    events, [weight_systs[0].input_collection]
                )
                if missing_fields:
                    continue
            ...
            if hasattr(weight_systs[0], "requires_branches"):
                if weight_systs[0].requires_branches is not None:
                    missing_fields = awkward_utils.missing_fields(
                        events, weight_systs[0].requires_branches
                    )
                    if missing_fields:
                        continue
            ...
            for idx, weight_syst in enumerate(weight_systs):
                ...
                if not hasattr(weight_syst, "target_collection"):
                    ...
                    missing_fields = awkward_utils.missing_fields(
                        events, [weight_syst.target_collection]
                    )
            ...
            for weight_name, weight_systs in self.weights.items():
                for weight_syst in weight_systs:
                    ...
                    if not weight_syst.is_produced:
                        reset = True
                        syst_events = weight_syst.produce(
                            syst_events, central_only=name != NOMINAL_TAG
                        )
                    ...
                    if hasattr(weight_syst, "modifies_taggers"):
                        ...
                        for tagger_name in weight_syst.modifies_taggers:
                            if tagger_name not in tag_idx_map.keys():
                                message = "[SystematicsProducer : apply_remaining_weight_syst] Weight systematic: %s was specified to modify the tagger '%s', but it is not in the list of tagger names we got from TagSequence. List is %s." % (
                                    weight_name,
                                    tagger_name,
                                    str(tag_idx_map.keys()),
                                )
                                logger.exception(message)
                                raise ValueError(message)
                            mask = (mask) | (
                                syst_events.tag_idx == tag_idx_map[tagger_name]
                            )
